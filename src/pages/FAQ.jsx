...
import faqCover from "../assets/faqCover.png"; // Import image

const faqs = [
  {
    question: "What types of courses does Seekio offer?",
    answer:
      "Seekio offers a wide range of courses tailored to the IT sector, including software development, data science, cybersecurity, cloud computing, and more. Each course is designed to equip students with industry-relevant skills and practical knowledge.",
  },
  {
  ...
  return (
    <div className="pt-20 relative bg-cover bg-center bg-no-repeat py-20 px-6">
      ...
      <div className="relative max-w-6xl mx-auto text-white">
        <h2 className="text-4xl font-bold mb-8  text-black">Frequently Asked Questions</h2>
        <div className="space-y-6">
          {faqs.map((faq, index) => (
            <div
              key={index}
              className={`bg-white bg-opacity-90 rounded-2xl overflow-hidden shadow-md ${openIndex === index ? 'ring-2 ring-purple-600' : ''}`}
            >
              <button
